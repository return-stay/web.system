...
export default {
  ...
  computed: {
    getTimeForam () {
      ...
      const stime = this.s_time
      console.log(stime)
      let timemoment = moment(stime).format('YYYY年')
      let time = timemoment || moment(new Date()).format("YYYY年MM月")
      return time
    }
  },
  data() {
    return {
      ...
      pickerOptions: {
        ...
        shortcuts: [{
          text: '本年',
          onClick(picker) {
            const [start, end] = DateUtil.getStartToEndYear()
            picker.$emit('pick', [start, end]);
          }
        }, {
          text: '上年',
          onClick(picker) {
            const [start, end] = DateUtil.getStartToEndLastYear()
            picker.$emit('pick', [start, end]);
          }
        }]
      },
  ...
  methods: {
    ...
    getInitTime() {
      ...
      let yearStartDate = this.dateToMs(new Date(year, 0, 1))
      this.s_time = yearStartDate
      this.dateString = moment(yearStartDate).format('YYYY年MM月DD日') + ' ~ ' +moment(etimeNumber).format('YYYY年MM月DD日');
    ...
    pickerChange(parmas) {
      ...
      if(data.length<2) {
        ...
        data = [yearStartDate, yearEndDate]
      }else {
        data = parmas
      }
